...
def load_results(progress_file: Path, results_file: Path) -> Dict:
    ...
    print(f"No results files found. Checked:")
...
def analyze_emergence_patterns(df: pd.DataFrame) -> Dict:
    ...
    if len(emerged_questions) > 0:
        analysis['emergence_steps'] = {
            'min': int(emerged_questions['emergence_step'].min()),
            'max': int(emerged_questions['emergence_step'].max()),
            'mean': float(emerged_questions['emergence_step'].mean()),
            'median': float(emerged_questions['emergence_step'].median()),
            'std': float(emerged_questions['emergence_step'].std())
        }
    ...
    for task in df['task'].unique():
        ...
        emerged_task = task_data[~task_data['always_correct'] & ~task_data['never_correct']]
        analysis['by_task'][task] = {
            'total': len(task_data),
            'always_correct': int(task_data['always_correct'].sum()),
            'never_correct': int(task_data['never_correct'].sum()),
            'mean_emergence_step': float(emerged_task['emergence_step'].mean()) if len(emerged_task) > 0 else None
        }

    # By emergence stage
    analysis['by_stage'] = {}
    for stage in df['emergence_stage'].unique():
        stage_data = df[df['emergence_stage'] == stage]
        analysis['by_stage'][stage] = {
            'total': len(stage_data),
            'always_correct': int(stage_data['always_correct'].sum()),
            'never_correct': int(stage_data['never_correct'].sum())
        }
...
def create_visualizations(df: pd.DataFrame, output_dir: Path):
    ...
    if len(emerged_questions) > 0:
        ...
        stage_counts = emerged_questions['emergence_stage'].value_counts()
        plt.bar(stage_counts.index, stage_counts.values)
        plt.xlabel('Training Stage')
        plt.ylabel('Number of Questions')
        plt.title('Question Emergence by Training Stage')
        plt.savefig(output_dir / 'emergence_by_stage.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    if 'emergence_steps' in analysis:
        print(f"- Emergence step range: {analysis['emergence_steps']['min']} → {analysis['emergence_steps']['max']}")
